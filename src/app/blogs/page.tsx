...
export default async function Blogs() {
    const blogs = await blogService.getPublicBlogs('-_id slug title description imageID createdAt tags');
                    ...
                    <a key={blog.slug} href={`/blogs/${blog.slug}`} className="block no-underline">
                        <div className="blog-card border-gray-500 border rounded-lg p-4 shadow-lg">
                            <div className="image-container w-full h-48 mb-4">
                                <CldImage src={blog.imageID} alt={blog.imageID} width="512" height="512" style={{ width: "100%", height: "100%" }} className="object-cover rounded-md w-full h-48 mb-4" />
                                {/*<img src="https://res.cloudinary.com/streakyfly/image/upload/c_limit,w_640/f_auto/q_auto/cld-sample-2?_a=BAVCr+DW0" alt={blog.imageID} className="object-cover rounded-md" />*/}
                            </div>
